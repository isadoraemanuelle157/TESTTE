...
export default {
  ...
  computed: {
    totalTracks() {
      // Placeholder - pode ser implementado com dados reais
  ...
  cantores: this.form.cantores
        ...
        if (this.modoEdicao) {
          ...
          this.showToast('Álbum atualizado com sucesso!')
        } else {
          await axios.post(API, payload)
          this.showToast('Álbum criado com sucesso!')
        }

        this.reset()
        this.carregarAlbuns()
      } catch (err) {
        this.showToast(err.response?.data?.error || 'Erro ao salvar álbum', 'error')
      } finally {
        this.saving = false
      }
    },
    ...
    async excluirConfirmado() {
      if (!this.albumParaExcluir) return
      this.deleting = true
      ...
      try {
        ...
        this.showToast('Erro ao excluir álbum', 'error')
      } finally {
        this.deleting = false
