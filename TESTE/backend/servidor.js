...
app.post('/rooms', requireAuth, async (req, res) => {
  try {
    const { id, name, description, isPublic, gradient, invitedUsers } = req.body
    ...
    const roomData = {
      ...
      gradient,
      createdBy: userId,
      source: 'spotify', // Logged users get Spotify
    ...
    if (invitedUsers?.length > 0) {
      ...
      for (const invitedId of invitedUsers) {
        await notificacaoService.criar({
          usuarioDestino: invitedId,
          usuarioOrigem: userId,
          tipo: 'room_invite',
          mensagem: `te convidou para a sala "${name}"`,
          dados: { roomId: id }
        })
      }
    }
...
// Get user's rooms
app.get('/rooms/my', requireAuth, async (req, res) => {
  try {
    const userId = req.user.id
...
// Get room by ID
app.get('/rooms/:id', optionalAuth, async (req, res) => {
  try {
...
// Invite user to room
app.post('/rooms/invite', requireAuth, async (req, res) => {
  try {
    ...
    if (room.createdBy !== inviterId) return res.status(403).json({ error: 'Sem permissão' })
    
    if (!room.invitedUsers) room.invitedUsers = []
    ...
    await notificacaoService.criar({
      ...
      usuarioOrigem: inviterId,
      tipo: 'room_invite',
      mensagem: `te convidou para a sala "${room.name}"`,
      dados: { roomId }
    })
...
// Join room
app.post('/rooms/:id/join', optionalAuth, async (req, res) => {
  try {
    const room = rooms.get(req.params.id)
    if (!room) return res.status(404).json({ error: 'Sala não encontrada' })
    ...
    if (!room.listeners.find(l => l.id === userId)) {
      room.listeners.push({
        id: userId,
    ...
    res.json({ room, listenerId: userId })
  } catch (error) {
    res.status(500).json({ error: error.message })
  }
})
